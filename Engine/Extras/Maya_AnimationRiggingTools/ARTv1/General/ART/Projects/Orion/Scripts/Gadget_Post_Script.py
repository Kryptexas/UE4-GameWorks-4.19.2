...
def scaleControls():
    ...
    for side in sides:
        ...
        cmds.select(["eyebrow_mid"+side+"_anim.cv[0:32]"])
...
# ------------------------------------------------------------------
def helmetStraps():
    const = cmds.parentConstraint("driver_jaw", "helmet_straps_l_ik_base_anim_grp", mo=True)[0]
    cmds.setAttr(const+".interpType", 2)
    const = cmds.parentConstraint("driver_jaw", "helmet_straps_r_ik_base_anim_grp", mo=True)[0]
    cmds.setAttr(const+".interpType", 2)
    const = cmds.parentConstraint("driver_jaw", "fk_helmet_straps_l_01_grp", mo=True)[0]
    cmds.setAttr(const+".interpType", 2)
    const = cmds.parentConstraint("driver_jaw", "fk_helmet_straps_r_01_grp", mo=True)[0]
    cmds.setAttr(const+".interpType", 2)
helmetStraps()
