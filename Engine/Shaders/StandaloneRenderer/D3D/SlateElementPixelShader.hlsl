...
float4 Main( VertexOut InVertex ) : SV_Target
{
	...
	float4 ClipCoords = InVertex.ClipCoords;
	float2 SSPosition = InVertex.WorldPosition.xy;
	clip( float4(SSPosition.x - ClipCoords.x, SSPosition.y - ClipCoords.y, ClipCoords.z - SSPosition.x, ClipCoords.w - SSPosition.y ) );
