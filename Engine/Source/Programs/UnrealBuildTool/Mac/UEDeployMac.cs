...
namespace UnrealBuildTool
{
	class UEDeployMac : UEBuildDeploy
	{
		...
		public static bool GeneratePList(string ProjectDirectory, bool bIsUE4Game, string GameName, string ProjectName, string InEngineDir, string ExeName)
		{
			...
			PListData = PListData.Replace("${EXECUTABLE_NAME}", ExeName).Replace("${APP_NAME}", GameName).Replace("${ICON_NAME}", GameName).Replace("${MACOSX_DEPLOYMENT_TARGET}", MacToolChain.MinMacOSVersionEditor).Replace("${BUNDLE_VERSION}", BundleVersion);
