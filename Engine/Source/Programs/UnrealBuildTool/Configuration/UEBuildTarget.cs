...
namespace UnrealBuildTool
{
	...
	public class UEBuildTarget
	{
		...
		/// </summary>
		/// <param name="Module">Game module to gather auto-startup list.</param>
		/// <returns>Auto-startup module name list.</returns>
		private IEnumerable<string> GetAutoStartupModuleList(UEBuildModuleCPP Module)
		{
		...
		private void CreateAutoStartupModuleListGetter()
		{
			if (TargetType != TargetRules.TargetType.Program)
			{
				foreach(var Module in ExtraModuleNames.Select(Name => GetModuleByName(Name)).Where(
					...
					).Cast<UEBuildModuleCPP>())
				{
					CreateAutoGeneratedModulesListForSpecificModule(Module);
				}
			}
			else
			{
				CreateAutoGeneratedModulesListForSpecificModule(
					GetModuleByName(AppName) as UEBuildModuleCPP);
			}
		}
		...
		/// Creates a getter that outputs auto-startup module list.
		/// </summary>
		/// <param name="Module">Module to create auto-startup list for.</param>
		private void CreateAutoGeneratedModulesListForSpecificModule(UEBuildModuleCPP Module)
		{
			var AutoStartupModuleArray = GetAutoStartupModuleList(Module).ToArray();

			var BaseFileName = "AutoStartupModuleListGetter";

			var ModuleDir = Path.Combine(Module.Binary.Config.IntermediateDirectory, BaseFileName);
			...
			Module.AddAdditionalCPPFiles(new FileItem[] { FileItem.GetItemByPath(SourceFilename) });
		}
		/// <summary>
		/// All non-program monolithic binaries implicitly depend on all static plugin libraries so they are always linked appropriately
		/// In order to do this, we create a new module here with a cpp file we emit that invokes an empty function in each library.
