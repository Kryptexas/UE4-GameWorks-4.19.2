...
public class HTML5Platform : Platform
{
    ...
    {
        Log("Post Staging File Copy {0}", Params.RawProjectPath);
        ...
        using (new PushedDirectory(Path.Combine(Params.BaseStageDirectory, "HTML5")))
        {
            ...
            if (Utils.IsRunningOnMono)
            {
                ...
                string CmdLine = string.Format("-c \" python {0} '{1}' --preload . --exclude \"*{2}\\Binaries\\HTML5\\*.*\" --js-output='{1}.js' \" ", PackagerPath, FinalDataLocation, Params.ShortProjectName);
    ...
    {
        // Archive everything in the Binaries for HTML5. 
        string PackagePath = Path.Combine(Path.GetDirectoryName(Params.RawProjectPath), "Binaries", "HTML5"); 
        SC.ArchiveFiles(PackagePath);
    }
