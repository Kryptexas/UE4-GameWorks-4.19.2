	...
	, bUseRelativePaths                     (bInUseRelativePaths)
{
	...
	int ExportAndTemporaryClassCount = 0;
	for (FClass* Cls : AllClasses.GetClassesInPackage(Package))
	{
		...
		bool bIsTemporaryClass = Cls->HasAnyClassFlags(CLASS_Temporary);
		if (bIsExportClass || bIsTemporaryClass)
		{
			ExportAndTemporaryClassCount = 1;
			break;
		}
	}
	...
	if (ExportAndTemporaryClassCount == 1)
	{
		FString PkgDir;
		FString GeneratedIncludeDirectory;
		...
		ClassesHeaderPath = GeneratedIncludeDirectory / PackageName + TEXT("Classes.h");
	...
	if (ExportAndTemporaryClassCount == 0)
	{
