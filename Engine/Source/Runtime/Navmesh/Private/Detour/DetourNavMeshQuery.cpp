...
dtStatus dtNavMeshQuery::findPath(dtPolyRef startRef, dtPolyRef endRef,
								  ...
								  float* pathSegmentCost, float* totalCost) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
...
//@UE4 BEGIN
	if (pathSegmentCost)
	{
		...
		do
		{
			pathSegmentCost[n] = (n > 0) ? (node->cost - pathSegmentCost[n - 1]) : 0.0f;
			n++;
...
//@UE4 END
	...
	*pathCount = n;
	if (totalCost) *totalCost = lastBestNode->total;
...
dtStatus dtNavMeshQuery::findClusterPath(dtPolyRef startRef, dtPolyRef endRef,
										 const float* startPos, const float* endPos,
										 const dtQueryFilter* filter,
										 dtPolyRef* path, int* pathCount, const int maxPath,
										 float* pathSegmentsCost, float* totalCost) const
{
	...
	*pathCount = 0;
	...
	{
		path[0] = startRef;
		if (pathSegmentsCost) pathSegmentsCost[0] = 0.0f;
		*pathCount = 1;
		return DT_SUCCESS;
	}
	
	const int maxClusterPath = 256;
	dtClusterRef clusterPath[maxClusterPath];
	int clusterPathSize;
	
	dtStatus status = findPathOnClusterGraph(startRef, endRef, filter, clusterPath, &clusterPathSize, maxClusterPath, totalCost);
	if (dtStatusSucceed(status))
	{
		status = findPathThroughClusters(startRef, endRef, startPos, endPos, filter, clusterPath, clusterPathSize, path, pathCount, maxPath, pathSegmentsCost);
	}
	else
	{
		*pathCount = 0;
...
#if TRACK_PATHFINDING_PERF
	m_queryTime = m_clusterTime + m_expandTime;	
#endif

	return status;
} 

int getIndexOnClusterPath(const dtMeshTile* tile, int polyIdx, dtClusterRef base, const dtClusterRef* clusterPath, int pathSize)
{
	const dtClusterRef cref = base | (dtClusterRef)tile->polyClusters[polyIdx];
	for (int i = 0; i < pathSize; i++)
		if (clusterPath[i] == cref)
			return i;

	return -1;
}

dtStatus dtNavMeshQuery::findPathThroughClusters(dtPolyRef startRef, dtPolyRef endRef,
												 const float* startPos, const float* endPos,
												 const dtQueryFilter* filter,
	...
	*pathCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		while (i != DT_NULL_LINK)
		{
			...
			// test with current poly when neighbour is an off-mesh connection
			const bool bOffMesh = (neighbourPoly->getType() != DT_POLYTYPE_GROUND);
			const dtMeshTile* clusterTestTile = bOffMesh ? bestTile : neighbourTile;
			dtPolyRef clusterTestRef = bOffMesh ? bestRef : neighbourRef;

			const int cindex = getIndexOnClusterPath(clusterTestTile, m_nav->decodePolyIdPoly(clusterTestRef),
													 m_nav->getClusterRefBase(clusterTestTile), clusterPath, clusterPathSize);
			...
			else
			{
				...
				// Heuristic guide towards center of next cluster
				const dtCluster* nextCluster = 0;
				if (cindex + 1 < clusterPathSize)
				{
					const dtMeshTile* nextClusterTile = m_nav->getTileByRef(clusterPath[cindex + 1]);
					nextCluster = &nextClusterTile->clusters[m_nav->decodePolyIdPoly(clusterPath[cindex + 1])];
				}

				if (nextCluster)
				{
					const float H_SCALE_C0 = H_SCALE * 0.5f;
					const float H_SCALE_E = H_SCALE * 0.5f;

					heuristic += dtVdist(neiPos, endPos)*H_SCALE_E;
					heuristic += dtVdist(neiPos, nextCluster->center)*H_SCALE_C0;
				}
				else
				{
					heuristic = dtVdist(neiPos, endPos)*H_SCALE;
...
#if TRACK_PATHFINDING_PERF
	const TimeVal endTime = getPerfTime();
	m_expandTime = getPerfDeltaTimeUsec(startTime, endTime) / 1000.0f;
#endif

	return status;
}

inline bool isValidClusterLink(const unsigned char flags, const bool bBacktracking)
{ 
	return bBacktracking ? ((flags & DT_CLINK_VALID_BCK) != 0) : ((flags & DT_CLINK_VALID_FWD) != 0);
}

dtStatus dtNavMeshQuery::findPathOnClusterGraph(dtPolyRef startRef, dtPolyRef endRef, const dtQueryFilter* filter,
												...
												float* totalCost) const
{
	...
	if (startTile->polyClusters == 0 || endTile->polyClusters == 0 ||
	...
	while (!m_openList->empty())
	{
		...
		while (i != DT_NULL_LINK)
		{
			// make sure that link has cost set
			m_nav->updateClusterLink(bestTile, bestClusterIdx, i, filter);

			const dtClusterLink& link = m_nav->getClusterLink(bestTile, i);
			const float linkCost = bBackTracking ? link.costBck : link.costFwd;
			i = link.next;

			const dtClusterRef& neighbourRef = link.ref;

			// Skip invalid ids and do not expand back to where we came from.
			if (!neighbourRef || neighbourRef == parentRef || linkCost < 0.0f)
				continue;

			// Check backtracking
			if (!isValidClusterLink(link.flags, bBackTracking))
				continue;
			...
			// Calculate cost and heuristic.
			const float cost = bestNode->cost + linkCost;
	...
	*clusterPathSize = n;
	if (totalCost) *totalCost = lastBestNodeCost;

#if TRACK_PATHFINDING_PERF
	const TimeVal endTime = getPerfTime();
	m_clusterTime = getPerfDeltaTimeUsec(startTime, endTime) / 1000.0f;
#endif
...
dtStatus dtNavMeshQuery::testClusterPath(dtPolyRef startRef, dtPolyRef endRef) const
{
	...
	while (!m_openList->empty())
	{
		...
		while (i != DT_NULL_LINK)
		{
			...
			// Skip invalid ids
			if ((link.flags & DT_CLINK_HASCOST_FWD) && link.costFwd < 0.0f)
				continue;
									  ...
									  int* straightPathCount, const int maxStraightPath, const int options) const
{
	...
	for (int i = startIdx; i < endIdx; i++)
	{
		...
		if (dtIntersectSegSeg2D(startPos, endPos, left, right, s, t))
		{
			...
			stat = appendVertex(pt, flags, path[i+1],
	...
	if (pathSize > 1)
	{
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (toType == DT_POLYTYPE_OFFMESH_SEGMENT)
			{
				...
				stat = appendVertex(portalApex, DT_STRAIGHTPATH_OFFMESH_CONNECTION, path[i+1],
					straightPath, straightPathFlags, straightPathRefs,
					straightPathCount, maxStraightPath);
...
dtStatus dtNavMeshQuery::findClustersInPathDistance(dtPolyRef startRef, const float* centerPos, const float pathDistance, const dtQueryFilter* filter,
													dtClusterRef* resultRef, int* resultCount, const int maxResult) const
{
	...
	*resultCount = 0;
	...
	const unsigned short startClusterIdx = startTile->polyClusters[startPolyIdx];
	dtClusterRef startClusterRef = m_nav->getClusterRefBase(startTile) | (dtClusterRef)startClusterIdx;
	const dtCluster& startCluster = startTile->clusters[startClusterIdx];
	// center pos is not being used, but it would require changing hierarchical graph to portals

	dtNode* startNode = m_nodePool->getNode(startClusterRef);
	dtVcopy(startNode->pos, startCluster.center);
	startNode->pidx = 0;
	startNode->cost = 0;
	startNode->total = 0;
	startNode->id = startClusterRef;
	startNode->flags = DT_NODE_OPEN;
	m_openList->push(startNode);
