...
public:
		...
		virtual void Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
		{
			if ( bTreeItemsAreDirty )
			{
				...
				if ( this->ItemsPanel.IsValid() )
				{
					...
					if ( OnGetChildren.IsBound() && TreeItemsSource != NULL )								
					{
			...
			bool bAddingItems )
		{
			...
			for ( int32 ItemIndex = 0; ItemIndex < ItemsSource.Num(); ++ItemIndex )
			{
				...
				const bool bIsExpanded = (CurItemInfo == NULL) ? false : CurItemInfo->bIsExpanded;
				bool bHasExpandedChildren = (CurItemInfo == NULL) ? false : CurItemInfo->bHasExpandedChildren;
	...
	protected:
		...
		/** The delegate that is invoked whnever an item in the tree is expanded or collapsed */
