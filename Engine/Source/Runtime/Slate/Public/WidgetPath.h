...
public:
	...
	FWidgetPath GetPathDownTo( TSharedRef<const SWidget> MarkerWidget ) const
	{
		...
		else
		{
			...
			return FWidgetPath( NULL, FArrangedChildren(EVisibility::Visible) );		
		}	
	}
...
class SLATE_API FWeakWidgetPath
{
	
public:
	/** Construct a weak widget path from a widget path. Defaults to an invalid path. */
	FWeakWidgetPath( const FWidgetPath& InWidgetPath = FWidgetPath( TSharedPtr<SWindow>(NULL), FArrangedChildren(EVisibility::Visible) ) );
