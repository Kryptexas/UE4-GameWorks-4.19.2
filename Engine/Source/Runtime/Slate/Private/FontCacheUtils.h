...
private:
	...
	FORCEINLINE void MarkAsRecent( CacheEntry* Entry )
	{
		...
		// The previous least recent item is now the most recent.  If it is NULL then this entry is the only item in the list
		if( Entry == LeastRecent && LeastRecent->Prev != NULL )
		{
