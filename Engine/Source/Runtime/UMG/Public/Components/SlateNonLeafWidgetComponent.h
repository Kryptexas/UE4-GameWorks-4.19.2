...
class UMG_API USlateNonLeafWidgetComponent : public USlateWrapperComponent
{
	...
	virtual void ReplaceChildAt(int32 Index, USlateWrapperComponent* Child)
	{
	}

	virtual bool AddChild(USlateWrapperComponent* Child, FVector2D Position)
	{
		return false;
	}

	virtual bool RemoveChild(USlateWrapperComponent* Child)
	{
		return false;
	}
};
