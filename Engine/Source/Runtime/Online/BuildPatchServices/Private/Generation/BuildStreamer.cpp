...
namespace FBuildPatchServices
{
	...
	private:
		...
		TRingBuffer< uint8, FBuildPatchServices::StreamBufferSize > BuildDataStream;
	...
	void FBuildStreamerImpl::ReadData()
	{
		...
		auto* StatPotentialThroughput = StatsCollector->CreateStat(TEXT("Stream: Potential Throughtput"), EStatFormat::DataSpeed);
		...
		uint8* FileReadBuffer = new uint8[FBuildPatchServices::FileBufferSize];
			...
			if (bBuildFileOpenSuccess)
			{
				...
				if (FileSize > 0)
				{
					...
					while (!FileReader->AtEnd() && !bShouldAbort)
					{
						...
						const uint32 ReadLen = FMath::Min< int64 >(FBuildPatchServices::FileBufferSize, SizeLeft);
						FStatsCollector::AccumulateTimeBegin(TempValue);
