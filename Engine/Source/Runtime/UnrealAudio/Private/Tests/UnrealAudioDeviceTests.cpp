...
namespace UAudio
{
	...
	bool TestDeviceQuery()
	{
		...
		if (!UnrealAudioDevice->GetNumInputDevices(NumInputDevices))
		{
			UE_LOG(LogUnrealAudio, Display, TEXT("Failed."));
			return false;
		}
		UE_LOG(LogUnrealAudio, Display, TEXT("Success: %d Number of Input Devices"), NumInputDevices);

		UE_LOG(LogUnrealAudio, Display, TEXT("Retrieving input device info for each input device..."), );
		for (uint32 DeviceIndex = 0; DeviceIndex < NumInputDevices; ++DeviceIndex)
		{
			FDeviceInfo DeviceInfo;
			if (!UnrealAudioDevice->GetInputDeviceInfo(DeviceIndex, DeviceInfo))
			{
				UE_LOG(LogUnrealAudio, Display, TEXT("Failed for index %d."), DeviceIndex);
				return false;
			}
	...
	*/
	...
	static bool DoInputTest(const FString& TestName, double LifeTime, Test::IGenerator* Generator)
	{
		check(Generator);
		...
		uint32 DefaultOutputDeviceIndex = 0;
		if (!UnrealAudioDevice->GetDefaultOutputDeviceIndex(DefaultOutputDeviceIndex))
		{
			UE_LOG(LogUnrealAudio, Display, TEXT("Failed to get default output device index."));
			return false;
		}

		FDeviceInfo OutputDeviceInfo;
		if (!UnrealAudioDevice->GetOutputDeviceInfo(DefaultOutputDeviceIndex, OutputDeviceInfo))
		{
			UE_LOG(LogUnrealAudio, Display, TEXT("Failed to get default output device info."));
			return false;
		}

		uint32 DefaultInputDeviceIndex = 0;
		if (!UnrealAudioDevice->GetDefaultInputDeviceIndex(DefaultInputDeviceIndex))
		{
			UE_LOG(LogUnrealAudio, Display, TEXT("Failed to get default input device index."));
			return false;
		}

		FDeviceInfo InputDeviceInfo;
		if (!UnrealAudioDevice->GetInputDeviceInfo(DefaultInputDeviceIndex, InputDeviceInfo))
		{
			UE_LOG(LogUnrealAudio, Display, TEXT("Failed to get default input device info."));
			return false;
		}

		FCreateStreamParams CreateStreamParams;
		CreateStreamParams.InputDeviceIndex = DefaultInputDeviceIndex;
		CreateStreamParams.OutputDeviceIndex = DefaultOutputDeviceIndex;
		CreateStreamParams.CallbackFunction = &TestCallback;
		CreateStreamParams.UserData = (void*)Generator;
	...
	bool TestDeviceInputSimple(double LifeTime)
	{
		Test::FPassThrough SimplePassThrough(LifeTime);
		return DoInputTest("input pass through", LifeTime, &SimplePassThrough);
	}
	...
	bool UNREALAUDIO_API TestDeviceInputRandomizedDelay(double LifeTime)
	{
		Test::FInputDelay SimpleDelayer(LifeTime);
		return DoInputTest("input randomized delay", LifeTime, &SimpleDelayer);
	}
