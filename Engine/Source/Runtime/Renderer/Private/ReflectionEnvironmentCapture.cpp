...
public:
	...
	void SetParameters(FRHICommandList& RHICmdList, int32 CubeFaceValue, int32 SourceMipIndexValue, float RoughnessValue, FSceneRenderTargetItem& SourceTextureValue)
	{
		FDownsamplePS::SetParameters(RHICmdList, CubeFaceValue, SourceMipIndexValue, SourceTextureValue);

		SetShaderValue(RHICmdList, GetPixelShader(), FilterRoughness, RoughnessValue);
...
void FilterReflectionEnvironment(FRHICommandList& RHICmdList, FSHVectorRGB3* OutIrradianceEnvironmentMap)
{
	const int32 EffectiveTopMipSize = GReflectionCaptureSize;
	const int32 NumMips = FMath::CeilLogTwo(EffectiveTopMipSize) + 1;
	...
	{	
		...
		for (int32 MipIndex = 0; MipIndex < NumMips; MipIndex++)
		{
			...
			for (int32 CubeFace = 0; CubeFace < CubeFace_MAX; CubeFace++)
			{
				...
				PixelShader->SetParameters(RHICmdList, CubeFace, MipIndex, Roughness, EffectiveSource);
...
void CaptureSceneToScratchCubemap(FRHICommandList& RHICmdList, FSceneRenderer* SceneRenderer, ECubeFace CubeFace, bool bCapturingForSkyLight, bool bLowerHemisphereIsBlack)
{
	...
	RHICmdList.CheckIsNull(); //FDeferredUpdateResource::UpdateResources()
...
void CaptureSceneIntoScratchCubemap(FScene* Scene, FVector CapturePosition, bool bCapturingForSkyLight, bool bStaticSceneOnly, float SkyLightNearPlane, bool bLowerHemisphereIsBlack, bool bCaptureEmissiveOnly)
{
	for (int32 CubeFace = 0; CubeFace < CubeFace_MAX; CubeFace++)
	{
			...
			bool, bLowerHemisphereIsBlack, bLowerHemisphereIsBlack,
		{
			...
			CaptureSceneToScratchCubemap(FRHICommandList::GetNullRef(), SceneRenderer, CubeFace, bCapturingForSkyLight, bLowerHemisphereIsBlack);
...
void FScene::UpdateSkyCaptureContents(const USkyLightComponent* CaptureComponent, bool bCaptureEmissiveOnly, FTexture* OutProcessedTexture, FSHVectorRGB3& OutIrradianceEnvironmentMap)
{
	if (FeatureLevel >= ERHIFeatureLevel::SM4)
	{
		...
		else if (CaptureComponent->SourceType == SLS_SpecifiedCubemap)
		{
			...
			{
				//@todo-rco: RHIPacketList
				CopyCubemapToScratchCubemap(FRHICommandList::GetNullRef(), SourceTexture, true, bLowerHemisphereIsBlack);
			});
		}
		else
		{
