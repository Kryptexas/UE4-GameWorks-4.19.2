...
public:
	...
	void SetMesh(FRHICommandList& RHICmdList, const FVertexFactory* VertexFactory, const FSceneView& View,
		const FPrimitiveSceneProxy* Proxy, const FMeshBatchElement& BatchElement, float DitheredLODTransitionValue)
	{
...
IMPLEMENT_SHADER_TYPE(, FFlexDownsampleSceneDepthPS, TEXT("DownsampleDepthPixelShader"), TEXT("Main"), SF_Pixel);
...
IMPLEMENT_SHADER_TYPE(, FFlexUpsampleSurfaceDepthPS, TEXT("FlexFluidSurfaceUpSampleShader"), TEXT("UpSampleMainPS"), SF_Pixel);
...
public:
	...
	*/
		...
		bool bDrawEllipsoids) const
	{
		...
		BaseVS->SetMesh(RHICmdList, VertexFactory, View, PrimitiveSceneProxy, BatchElement, DitheredLODTransitionValue);
		BasePS->SetMesh(RHICmdList, VertexFactory, View, PrimitiveSceneProxy, BatchElement, DitheredLODTransitionValue);

		FMeshDrawingPolicy::SetMeshRenderState(RHICmdList, View, PrimitiveSceneProxy, Mesh, BatchElementIndex, bBackFace,
			DitheredLODTransitionValue, FMeshDrawingPolicy::ElementDataType(), PolicyContext);
	}
		...
		)
	{
		//Draw depths based on particles
		{
			...
			DrawingPolicy.SetMeshRenderState(RHICmdList, View, PrimitiveSceneProxy, Mesh, BatchElementIndex, bBackFace,
				Mesh.DitheredLODTransitionAlpha, FFlexFluidSurfaceDrawingPolicy::ContextDataType(),
				DrawingContext.bThicknessPass, DrawingContext.bDrawEllipsoids);
...
void AllocateTexturesIfNecessary(FRHICommandList& RHICmdList, FFlexFluidSurfaceTextures& Textures, FIntPoint NewBufferSize, FIntPoint SceneBufferSize)
{
	...
	if (NewBufferSize != Textures.BufferSize)
	{
		...
		if (Textures.BufferSize.X > 0 && Textures.BufferSize.Y > 0)
		{
			...
			{
				FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(Textures.BufferSize, PF_DepthStencil, FClearValueBinding::Transparent, TexCreate_None, TexCreate_DepthStencilTargetable, false));
...
void ClearTextures(FRHICommandList& RHICmdList, FFlexFluidSurfaceTextures& Textures, const FViewInfo& View)
{
	//Clear depth buffers
	{
		...
		DrawClearQuad(RHICmdList, View.GetFeatureLevel(), true, FLinearColor(FVector(ViewDepth)), true, Depth, false, 0);
	}
...
void SmoothDepth(FRHICommandList& RHICmdList, const FViewInfo& View, FFlexFluidSurfaceSceneProxy* SurfaceSceneProxy)
{
	...
	if (SurfaceSceneProxy->MaxRadialSamples == 1 || SurfaceSceneProxy->SmoothingRadius == 0.0f)
	{
		...
		CopyVertexShader->SetParameters(RHICmdList, View);
