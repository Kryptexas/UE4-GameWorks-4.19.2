...
DECLARE_GPU_STAT(BeginAccumulation);
DECLARE_GPU_STAT(RenderVolume);
DECLARE_GPU_STAT(EndAccumulation);
DECLARE_GPU_STAT(ApplyLighting);
...
void FDeferredShadingSceneRenderer::NVVolumetricLightingBeginAccumulation(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		SCOPED_DRAW_EVENT(RHICmdList, BeginAccumulation);
		SCOPED_GPU_STAT(RHICmdList, BeginAccumulation);
...
void FDeferredShadingSceneRenderer::NVVolumetricLightingRenderVolume(FRHICommandListImmediate& RHICmdList, const FLightSceneInfo* LightSceneInfo)
{
	...
	SCOPED_DRAW_EVENT(RHICmdList, RenderVolume);
	SCOPED_GPU_STAT(RHICmdList, RenderVolume);
...
void FDeferredShadingSceneRenderer::NVVolumetricLightingEndAccumulation(FRHICommandListImmediate& RHICmdList)
{
	...
	SCOPED_DRAW_EVENT(RHICmdList, EndAccumulation);
	SCOPED_GPU_STAT(RHICmdList, EndAccumulation);
...
void FDeferredShadingSceneRenderer::NVVolumetricLightingApplyLighting(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		SCOPED_DRAW_EVENT(RHICmdList, ApplyLighting);
		SCOPED_GPU_STAT(RHICmdList, ApplyLighting);
