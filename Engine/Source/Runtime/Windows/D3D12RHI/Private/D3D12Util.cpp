...
namespace D3D12RHI
{
	void VerifyD3D11Result(HRESULT D3DResult, const ANSICHAR* Code, const ANSICHAR* Filename, uint32 Line, ID3D12Device* Device)
	{
		...
		{
			FPlatformMisc::MessageBoxExt(EAppMsgType::Ok, *NSLOCTEXT("D3D12RHI", "OutOfMemory", "Out of video memory trying to allocate a rendering resource.").ToString(), TEXT("Error"));
#if STATS
...
void FD3D12DynamicBuffer::InitRHI()
{
}

void FD3D12DynamicBuffer::ReleaseRHI()
{
}
...
DEFINE_STAT(STAT_D3D12ApplyStatePSOCreateTime);
