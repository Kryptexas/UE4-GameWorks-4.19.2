		...
		for (int32 OverlapIndex=0; OverlapIndex < Overlaps.Num(); ++OverlapIndex)
		{
			...
			for (int ShapeIndex = 0; ShapeIndex < Shapes.Num(); ++ShapeIndex)
			{
				...
				if (CollisionComponent == nullptr || !CollisionComponent->bIsLocalSimParent)
				{
					...
					const PxVec3 AngularVelocity = U2PVector(Body->GetUnrealWorldAngularVelocity_AssumesLocked());

					// generate finite transform from velocities
					const float RadiansPerSecond = FMath::DegreesToRadians(AngularVelocity.magnitude());
					const float Dt = AverageDeltaTime;

					DeltaTransform = PxTransform(-LinearVelocity*Dt, PxQuat(-RadiansPerSecond*Dt, AngularVelocity.getNormalized()));
				}
