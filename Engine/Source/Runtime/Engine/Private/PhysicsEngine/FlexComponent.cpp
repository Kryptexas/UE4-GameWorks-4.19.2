	...
	if (Proxy && GetStaticMesh()->FlexAsset->GetClass() == UFlexAssetSoft::StaticClass())
	{
		...
		else
		{
			...
			else
			{
				...
				for (int i = 0; i < NumShapes; ++i)
				{
					NewTransforms[i].Translation = ComponentToWorld.TransformPosition(GetStaticMesh()->FlexAsset->ShapeCenters[i]);
...
void UFlexComponent::EnableSim()
{
	...
	if (ContainerInstance && !AssetInstance)
	{
		...
		AssetInstance = ContainerInstance->CreateInstance(Asset, ComponentToWorld.ToMatrixNoScale(), FVector(0.0f), ContainerInstance->GetPhase(Phase));
...
void UFlexComponent::UpdateSimPositions()
{
	...
	if (NumParticles == PreSimPositions.Num())
	{
		...
		ComponentToWorld = PreSimTransform;
	}
	else
	{
...
void UFlexComponent::SetGravity(FVector Gravity)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Gravity = Gravity;
	}
}

void UFlexComponent::SetWind(FVector Wind)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Wind = Wind;
	}
}

void UFlexComponent::SetRadius(float Radius)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Radius = Radius;
	}
}

void UFlexComponent::SetViscosity(float Viscosity)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Viscosity = Viscosity;
	}
}

void UFlexComponent::SetShapeFriction(float ShapeFriction)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->ShapeFriction = ShapeFriction;
	}
}

void UFlexComponent::SetParticleFriction(float ParticleFriction)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->ParticleFriction = ParticleFriction;
	}
}

void UFlexComponent::SetSleepThreshold(float SleepThreshold)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->SleepThreshold = SleepThreshold;
	}
}

void UFlexComponent::SetMaxVelocity(float MaxVelocity)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->MaxVelocity = MaxVelocity;
	}
}

void UFlexComponent::SetRelaxationFactor(float RelaxationFactor)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->RelaxationFactor = RelaxationFactor;
	}
}

void UFlexComponent::SetDamping(float Damping)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Damping = Damping;
	}
}

void UFlexComponent::SetCollisionDistance(float CollisionDistance)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->CollisionDistance = CollisionDistance;
	}
}

void UFlexComponent::SetRestitution(float Restitution)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Restitution = Restitution;
	}
}

void UFlexComponent::SetAdhesion(float Adhesion)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Adhesion = Adhesion;
	}
}

void UFlexComponent::SetShockPropagation(float ShockPropagation)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->ShockPropagation = ShockPropagation;
	}
}

void UFlexComponent::SetDissipation(float Dissipation)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Dissipation = Dissipation;
	}
}

void UFlexComponent::SetDrag(float Drag)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Drag = Drag;
	}
}

void UFlexComponent::SetLift(float Lift)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Lift = Lift;
	}
}

void UFlexComponent::SetRestDistance(float RestDistance)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->RestDistance = RestDistance;
	}
}

void UFlexComponent::SetCohesion(float Cohesion)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->Cohesion = Cohesion;
	}
}

void UFlexComponent::SetSurfaceTension(float SurfaceTension)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->SurfaceTension = SurfaceTension;
	}
}

void UFlexComponent::SetVorticityConfinement(float VorticityConfinement)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->VorticityConfinement = VorticityConfinement;
	}
}

void UFlexComponent::SetSolidPressure(float SolidPressure)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->SolidPressure = SolidPressure;
	}
}

void UFlexComponent::SetAnisotropyScale(float AnisotropyScale)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->AnisotropyScale = AnisotropyScale;
	}
}

void UFlexComponent::SetAnisotropyMin(float AnisotropyMin)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->AnisotropyMin = AnisotropyMin;
	}
}

void UFlexComponent::SetAnisotropyMax(float AnisotropyMax)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->AnisotropyMax = AnisotropyMax;
	}
}

void UFlexComponent::SetPositionSmoothing(float PositionSmoothing)
{
	if (ContainerInstance)
	{
		ContainerInstance->Template->PositionSmoothing = PositionSmoothing;
	}
}
