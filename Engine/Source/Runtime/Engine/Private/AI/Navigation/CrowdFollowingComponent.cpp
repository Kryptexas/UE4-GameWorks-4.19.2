...
DEFINE_LOG_CATEGORY_STATIC(LogCrowdFollowing, Warning, All);
...
void UCrowdFollowingComponent::UpdateCachedDirections(const FVector& NewVelocity, const FVector& NextPathCorner)
{
...
void UCrowdFollowingComponent::ApplyCrowdAgentVelocity(const FVector& NewVelocity, const FVector& DestPathCorner)
{
	if (bEnableCrowdSimulation)
	{
			...
			(bAffectFallingVelocity || CharacterMovement == NULL || CharacterMovement->MovementMode != MOVE_Falling))
		{
			...
			UpdateCachedDirections(NewVelocity, DestPathCorner);
		}
	}
}
...
void LogPathPartHelper(AActor* LogOwner, FNavMeshPath* NavMeshPath, int32 StartIdx, int32 EndIdx)
{
	...
	for (int32 Idx = StartIdx + 1; Idx < EndIdx; Idx++)
	{
		...
		UE_VLOG_SEGMENT_THICK(LogOwner, SegmentStart, SegmentEnd, FColor::Yellow, 2, TEXT_EMPTY);
...
void UCrowdFollowingComponent::SetMoveSegment(int32 SegmentStartIndex)
{
	...
	if (NavMeshPath)
	{
		...
		UE_VLOG_SEGMENT(GetOwner(), MovementComp->GetActorFeetLocation(), CurrentTargetPt, FColor::Red, TEXT("path part"));
	...
	else
	{
		...
		UE_VLOG_SEGMENT(GetOwner(), AgentLoc, CurrentTargetPt, FColor::Red, TEXT("path"));
