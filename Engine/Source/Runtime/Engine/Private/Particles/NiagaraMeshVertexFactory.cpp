...
public:
	...
	virtual void SetMesh(FRHICommandList& RHICmdList, FShader* Shader, const FVertexFactory* VertexFactory, const FSceneView& View, const FMeshBatchElement& BatchElement, uint32 DataFlags) const override
	{
		...
		if (!bInstanced)
		{
			const FNiagaraMeshVertexFactory::FBatchParametersCPU* BatchParameters = (const FNiagaraMeshVertexFactory::FBatchParametersCPU*)BatchElement.UserData;
			const FNiagaraMeshInstanceVertex* Vertex = BatchParameters->InstanceBuffer + BatchElement.UserIndex;
			const FNiagaraMeshInstanceVertexDynamicParameter* DynamicVertex = BatchParameters->DynamicParameterBuffer + BatchElement.UserIndex;
			const FNiagaraMeshInstanceVertexPrevTransform* PrevTransformVertex = BatchParameters->PrevTransformBuffer + BatchElement.UserIndex;
