...
void UByteProperty::SerializeItem( FArchive& Ar, void* Value, void const* Defaults ) const
{
	...
	else if (Ar.IsLoading())
	{
		...
		*(uint8*)Value = Enum->GetValueByName(EnumValueName);
		if ( Enum->GetMaxEnumValue() + 1 < *(uint8*)Value )
		{
	...
	else
	{
		...
		// is the _MAX entry
		if ( ByteValue < Enum->GetMaxEnumValue() )
		{
...
void UByteProperty::ExportTextItem( FString& ValueStr, const void* PropertyValue, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope ) const
{
	if (0 != (PortFlags & PPF_ExportCpp))
	{
		if (Enum)
		{
			...
			const int32 GoodValue = (ActualValue > MaxValue) ? MaxValue : ActualValue;
	...
	if( Enum && (PortFlags & PPF_ConsoleVariable) == 0 )
	{
		...
		if (*(const uint8*)PropertyValue <= Enum->GetMaxEnumValue() - 1 || ((PortFlags&PPF_Copy) != 0 && *(const uint8*)PropertyValue <= Enum->GetMaxEnumValue()))
		{
