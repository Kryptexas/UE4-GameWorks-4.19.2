...
void FNiagaraCollisionBatch::KickoffNewBatch(FNiagaraSimulation *Sim, float DeltaSeconds)
{
	...
	UWorld *EffectWorld = Sim->GetParentEffectInstance()->GetComponent()->GetWorld();
	if (EffectWorld)
	{
...
void FNiagaraCollisionBatch::GenerateEventsFromResults(FNiagaraSimulation *Sim)
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraCollision);
