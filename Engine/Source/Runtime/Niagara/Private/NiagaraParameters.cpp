...
FNiagaraVariable* FNiagaraParameters::FindParameter(FGuid InParamGuid)
{
	auto* C = Parameters.FindByPredicate([&](const FNiagaraVariable& Param) { return Param.GetId() == InParamGuid; });
	return C;
}

const FNiagaraVariable* FNiagaraParameters::FindParameter(FGuid InParamGuid) const
{
	auto* C = Parameters.FindByPredicate([&](const FNiagaraVariable& Param) { return Param.GetId() == InParamGuid; });
	return C;
}
