...
bool FALAudioDevice::InitializeHardware( void )
{
	...
	//UE_LOG(LogALAudio, Log, TEXT( "ALAudio: no OpenAL devices found." ) );
	//UE_LOG(LogALAudio, Log, TEXT( "ALAudio device requested : %s" ), *DeviceName );
	//UE_LOG(LogALAudio, Log, TEXT( "ALAudio device Opened : %s" ), OpenedDeviceName );
...
bool FALAudioDevice::FindExt( const TCHAR* Name )
{
	...
	{
		//UE_LOG ( LogALAudio, log,TEXT( "Device supports: %s" )); 
		return( true );
	}
	return( true );
}
