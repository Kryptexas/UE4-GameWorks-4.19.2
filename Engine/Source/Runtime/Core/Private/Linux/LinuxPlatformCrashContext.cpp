...
void GenerateWindowsErrorReport(const FString & WERPath, bool bReportingNonCrash)
{
	...
	if (ReportFile != NULL)
	{
		...
		WriteLine(ReportFile, *FString::Printf(TEXT("\t\t<Parameter0>%s</Parameter0>"), *FGenericCrashContext::GetCrashGameName()));
		...
		WriteLine(ReportFile, *FString::Printf(TEXT("\t\t<CrashType>%s</CrashType>"), FGenericCrashContext::GetCrashTypeString(bReportingNonCrash, FDebug::HasAsserted(), GIsGPUCrashed)));
