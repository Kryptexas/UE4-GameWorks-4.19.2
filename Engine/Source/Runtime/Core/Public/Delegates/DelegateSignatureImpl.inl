...
public:
	...
	void __Internal_BindDynamic( UserClass* InUserObject, typename TMethodPtrResolver< UserClass >::FMethodPtr InMethodPtr, const TCHAR* InMacroFunctionName )
	{
		...
		const TCHAR* TrimmedMacroFunctionName = FCString::Strrstr( InMacroFunctionName, TEXT( "::" ) );
		checkf(TrimmedMacroFunctionName, TEXT("'%s' does not look like a member function"), InMacroFunctionName);
		this->FunctionName = TrimmedMacroFunctionName + 2;
