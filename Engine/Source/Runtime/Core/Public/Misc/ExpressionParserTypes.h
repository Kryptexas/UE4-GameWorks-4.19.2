...
public:
	...
	template<typename T, typename U = typename TEnableIf<!TPointerIsConvertibleFromTo<T, FExpressionNode>::Value>::Type>
	FExpressionNode(const T& In) : TypeId(TGetExpressionNodeTypeId<T>::GetTypeId())
	{
...
namespace impl
{
	...
	template <typename T>
	struct TParams : TParamsImpl<decltype(&T::operator())> {};
}
