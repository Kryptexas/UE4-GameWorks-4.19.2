...
struct AIMODULE_API FActorPerceptionInfo
{
	...
	FActorPerceptionInfo(class AActor* InTarget = NULL)
...
protected:	
	struct FStimulusToProcess
	{
		...
		FStimulusToProcess(class AActor* InSource, const FAIStimulus& InStimulus)
...
public:
	...
	const AActor* GetCollisionActor() const;
	...
	FVector GetActorLocation(const class AActor* Actor) const;
	FORCEINLINE const FActorPerceptionInfo* GetActorInfo(const class AActor* Actor) const { return PerceptualData.Find(Actor); }	
	...
	void RegisterStimulus(class AActor* Source, const FAIStimulus& Stimulus);
	void ProcessStimuli();
	void AgeStimuli(const float ConstPerceptionAgingRate);
	void ForgetActor(class AActor* ActorToForget);

	float GetYoungestStimulusAge(const class AActor* Source) const;
	bool HasAnyActiveStimulus(const class AActor* Source) const;
	bool HasActiveStimulus(const class AActor* Source, FAISenseId Sense) const;
