	...
	if ( Objects.Num() > 0 )
	{
		...
		if ( AssetRegistryModule.Get().IsLoadingAssets() )
		{
			...
			SDiscoveringAssetsDialog::OpenDiscoveringAssetsDialog(
				SDiscoveringAssetsDialog::FOnAssetsDiscovered::CreateSP(this, &FAssetTypeActions_Redirector::FixupReferencers, Objects)
				);
		}
		else
		{
			// No need to wait, attempt to fix references now.
			FixupReferencers(Objects);
		}
	}
}
...
void FAssetTypeActions_Redirector::FixupReferencers(TArray<TWeakObjectPtr<UObjectRedirector>> Objects) const
{
	...
	for (auto ObjIt = Objects.CreateConstIterator(); ObjIt; ++ObjIt)
	{
		...
		if ( Object )
		{
			new (RedirectorRefsList) FRedirectorRefs(Object);
		}
	}
...
void FAssetTypeActions_Redirector::PopulateRedirectorReferencers(TArray<FRedirectorRefs>& RedirectorsToPopulate) const
{
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
...
bool FAssetTypeActions_Redirector::UpdatePackageStatus(const TArray<FRedirectorRefs>& RedirectorsToFix) const
{
...
void FAssetTypeActions_Redirector::LoadReferencingPackages(TArray<FRedirectorRefs>& RedirectorsToFix, TArray<UPackage*>& OutReferencingPackagesToSave) const
{
	const bool bAllowNewSlowTask = true;
	FStatusMessageContext SlowTaskMessage( LOCTEXT( "LoadingReferencingPackages", "Loading Referencing Packages..." ), bAllowNewSlowTask );
	ISourceControlProvider& SourceControlProvider = ISourceControlModule::Get().GetProvider();
...
bool FAssetTypeActions_Redirector::CheckOutReferencingPackages(TArray<FRedirectorRefs>& RedirectorsToFix, TArray<UPackage*>& InOutReferencingPackagesToSave) const
{
...
void FAssetTypeActions_Redirector::DetectReadOnlyPackages(TArray<FRedirectorRefs>& RedirectorsToFix, TArray<UPackage*>& InOutReferencingPackagesToSave) const
{
...
void FAssetTypeActions_Redirector::SaveReferencingPackages(const TArray<UPackage*>& ReferencingPackagesToSave) const
{
...
void FAssetTypeActions_Redirector::DeleteRedirectors(TArray<FRedirectorRefs>& RedirectorsToFix) const
{
...
void FAssetTypeActions_Redirector::ReportFailures(const TArray<FRedirectorRefs>& RedirectorsToFix) const
{
