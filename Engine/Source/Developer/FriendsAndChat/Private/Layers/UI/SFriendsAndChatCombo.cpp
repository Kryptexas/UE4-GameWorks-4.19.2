...
public:
	...
	{}
		...
		SLATE_ARGUMENT(const FFriendsComboStyle*, ComboStyle)
	...
	void Construct(const FArguments& InArgs)
	{
		...
		const FComboButtonStyle* ComboButtonStyle = InArgs._ButtonStyleOverride != nullptr ? InArgs._ButtonStyleOverride->ComboButtonStyle : &ComboStyle.FriendListComboButtonStyle;
		const FTextBlockStyle* ComboTextStyle = InArgs._ButtonStyleOverride != nullptr ? InArgs._ButtonStyleOverride->ButtonTextStyle : &ComboStyle.FriendsComboTextStyle;
...
private:
	...
	TSharedRef<SWidget> GetMenuContent()
	{
		...
		for (int32 Idx = 0; Idx < DropdownItemsRef.Num(); ++Idx)
		{
			...
			.AutoHeight()
			.Padding(ComboStyle.ComboItemPadding)
			[
				...
				.ButtonStyle(&ComboStyle.ComboItemButtonStyle)
				.ContentPadding(ComboStyle.ComboItemContentPadding)
				...
				[
					...
					.TextStyle(&ComboStyle.FriendsComboTextStyle)
		...
		if (Placement.Get() == MenuPlacement_ComboBoxRight)
		{
			...
			[
				SNew(SImage)
				.Image(&ComboStyle.FriendComboBackgroundRightFlippedBrush)
			];
			...
			[
				...
				[
					SNew(SImage)
					.Image(&ComboStyle.FriendComboBackgroundLeftFlippedBrush)
				]
			];
		}
		else
		{
			...
			[
				...
				[
					SNew(SImage)
					.Image(&ComboStyle.FriendComboBackgroundLeftBrush)
				]
			];
			...
			[
				SNew(SImage)
				.Image(&ComboStyle.FriendComboBackgroundRightBrush)
			];
		}
		...
		[
			SNew(SBox)
			.Padding(ComboStyle.ComboMenuPadding)
			[
	...
	FSlateColor GetTextColor(int32 Idx) const
	{
		...
		{
			return FSlateColor(ComboStyle.ComboItemTextColorInverted);
		}
		return FSlateColor(ComboStyle.ComboItemTextColorNormal);
	}
