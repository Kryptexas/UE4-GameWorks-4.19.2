...
TSharedRef< FTimeStampRun > FTimeStampRun::Create(const TSharedRef<class FTextLayout>& TextLayout, const FRunInfo& InRunInfo, const TSharedRef< const FString >& InText, const FTimeStampRunInfo& InWidgetInfo)
{
	TSharedRef<FTimeStampRun> TimeStampRun = MakeShareable(new FTimeStampRun(TextLayout, InRunInfo, InText, InWidgetInfo));
...
FVector2D FTimeStampRun::Measure( int32 StartIndex, int32 EndIndex, float Scale ) const 
{
	return Info.Size.Get( TimeStampWidget->GetDesiredSize() ) * Scale;
}

int8 FTimeStampRun::GetKerning( int32 CurrentIndex, float Scale ) const 
{
	return 0;
}

TSharedRef< ILayoutBlock > FTimeStampRun::CreateBlock( int32 StartIndex, int32 EndIndex, FVector2D Size, const TSharedPtr< IRunRenderer >& Renderer )
{
...
int32 FTimeStampRun::OnPaint( const FPaintArgs& Args, const FTextLayout::FLineView& Line, const TSharedRef< ILayoutBlock >& Block, const FTextBlockStyle& DefaultStyle, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const 
{
	...
	return TimeStampWidget->Paint( Args, WidgetGeometry, MyClippingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled );
}
...
{
	return FTimeStampRun::Create(TextLayout.Pin().ToSharedRef(), RunInfo, Text, Info);
}
...
FTimeStampRun::FTimeStampRun(const TSharedRef<class FTextLayout>& InTextLayout, const FRunInfo& InRunInfo, const TSharedRef< const FString >& InText, const FTimeStampRunInfo& InWidgetInfo)
