...
public:
	 ...
	 */
	UPROPERTY(GlobalConfig, EditAnywhere, Category=Emscripten, Meta = (DisplayName = "asmjs Build (else build WASM [EXPERIMENTAL: REQUIRES UE4 SOURCE CODE!])"))
	bool TargetAsmjs;
	 ...
	 */
	UPROPERTY(GlobalConfig, EditAnywhere, Category=Emscripten, Meta = (DisplayName = "IndexedDB storage (WASM only - during shipping packaging [EXPERIMENTAL: REQUIRES UE4 SOURCE CODE!])"))
