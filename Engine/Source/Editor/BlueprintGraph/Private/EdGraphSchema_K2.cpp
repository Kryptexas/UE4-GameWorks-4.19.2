...
{
	if (!InPin.bUseBackwardsCompatForEmptyAutogeneratedValue)
	{
		if (InPin.PinType.PinCategory == PC_Enum || InPin.PinType.PinCategory == PC_Byte)
		{
			...
			if (InPin.AutogeneratedDefaultValue.IsEmpty())
			{
				...
				else if (InPin.PinType.PinCategory == PC_Byte && FCString::Atoi(*PinDefaultValue) == 0)
				{
					return true;
				}
			}
		}
		else if (InPin.PinType.PinCategory == PC_Float)
		{
