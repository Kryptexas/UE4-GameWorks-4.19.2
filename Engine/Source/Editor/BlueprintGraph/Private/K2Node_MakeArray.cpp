...
void UK2Node_MakeArray::AllocateDefaultPins()
{
	...
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Wildcard, FString(), nullptr, *OutputPinName, EPinContainerType::Array);
	...
	else
	{
		...
		if (NumPinsWithLinks == 0)
		{
			...
			for (int32 PinIndex = 0; PinIndex < InputPins.Num(); ++PinIndex)
			{
				...
				if (!CurrentPin->DoesDefaultValueMatchAutogenerated())
				{
...
void UK2Node_MakeArray::AddInputPin()
{
	...
	UEdGraphPin* Pin = CreatePin(EGPD_Input, OutputPinType.PinCategory, OutputPinType.PinSubCategory, OutputPinType.PinSubCategoryObject.Get(), *FString::Printf(TEXT("[%d]"), (NumInputs-1)));
