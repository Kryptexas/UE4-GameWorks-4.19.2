...
/** Blueprint Compiler map*/
TMap<TSubclassOf<UBlueprint>, FOnGetBlueprintCompiler> FBlueprintEditorModule::BlueprintCompilers;

void FBlueprintEditorModule::RegisterCompilerGetter(const TSubclassOf<UBlueprint> BlueprintType, FOnGetBlueprintCompiler InCompilerGetter)
{
	if (InCompilerGetter.IsBound())
	{
		FOnGetBlueprintCompiler* Getter = BlueprintCompilers.Find(BlueprintType);
		if (Getter)
		{
			*Getter = InCompilerGetter;
		}
		else
		{
			BlueprintCompilers.Add(BlueprintType, InCompilerGetter);
		}
	}
}

void FBlueprintEditorModule::UnregisterCompilerGetter(const TSubclassOf<UBlueprint> BlueprintType)
{
	if (UObjectInitialized())
	{
		BlueprintCompilers.Remove(BlueprintType);
	}
}

TSharedPtr<FKismetCompilerContext> FBlueprintEditorModule::GetCompiler(UBlueprint* BP, FCompilerResultsLog& InMessageLog, const FKismetCompilerOptions& InCompileOptions)
{
	if (BP != nullptr)
	{
		...
		if (BlueprintCompilers.Num() > 0)
		{
			...
			{
				if (FOnGetBlueprintCompiler* CompilerGetter = BlueprintCompilers.Find(Class))
				{
					return CompilerGetter->Execute(BP, InMessageLog, InCompileOptions);
				}
			}
		}
	}
