...
void SNiagaraCurveEditor::Construct(const FArguments& InArgs, TSharedRef<FNiagaraEffectViewModel> InEffectViewModel)
{
	EffectViewModel = InEffectViewModel;
	EffectViewModel->OnCurveOwnerChanged().AddRaw(this, &SNiagaraCurveEditor::OnCurveOwnerChanged);
	InputSnap = .1f;
	OutputSnap = .1f;

	SAssignNew(CurveEditor, SCurveEditor)
		.ShowCurveSelector(true);
	CurveEditor->SetCurveOwner(&EffectViewModel->GetCurveOwner());
...
{
	EffectViewModel->OnCurveOwnerChanged().RemoveAll(this);
}
