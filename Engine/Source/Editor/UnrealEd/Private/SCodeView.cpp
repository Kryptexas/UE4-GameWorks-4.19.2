...
namespace CodeView
{
	...
	namespace Helpers
	{
		...
		FString MakeCustomColumnComboText( const ECustomColumnMode::Type& Mode )
		{
			FString ModeName;
			switch( Mode )
			{
			case ECustomColumnMode::None:
				ModeName = LOCTEXT("CustomColumnMode_None", " - ").ToString();
				break;

			case ECustomColumnMode::ModuleName:
				ModeName = LOCTEXT("CustomColumnMode_ModuleName", "Module").ToString();
				break;
	...
	bool SCodeView::bReadyToPopulate = false;
	...
	void SCodeView::Construct( const FArguments& InArgs )
	{
		...
		struct Local
		{	
			static FString MakeCustomColumnComboToolTipText( const ECustomColumnMode::Type& Mode )
			{
				FString ToolTipText;
				switch( Mode )
				{
					case ECustomColumnMode::None:
						ToolTipText = LOCTEXT("CustomColumnModeToolTip_None", "Hides all extra function info").ToString();
						break;

					case ECustomColumnMode::ModuleName:
						ToolTipText = LOCTEXT("CustomColumnModeToolTip_ModuleName", "Displays the name of the module each function resides within").ToString();
						break;
					...
					[
						SNew( SHorizontalBox )
							+SHorizontalBox::Slot()
								...
								[
									SNew( STextBlock )
										.Text( LOCTEXT("TreeColumn_FunctionLabel", "Function").ToString() )
								]
					]
					...
					[
						SNew( SHorizontalBox )
							+SHorizontalBox::Slot()
								...
								[
									SNew( STextBlock )
										.Text( LOCTEXT("TreeColumn_CustomColumn", "Info").ToString() )
								]
							...
							+SHorizontalBox::Slot()
								...
								[
									SAssignNew( CustomColumnModeComboBox, SCustomColumnModeComboBoxType )
										...
										.ToolTipText( LOCTEXT("CustomColumnModeComboBox_ToolTip", "Choose what type of information to display in this column").ToString() )
					...
					SNew( SVerticalBox )
						...
						[
							SAssignNew( FilterTextBoxWidget, SSearchBox )
								.ToolTipText( LOCTEXT("FilterSearchHint", "Type here to search functions").ToString() )
	...
	FString SCodeView::GetFilterStatusText() const
	{
		if( SignatureToTreeItemMap.Num() == 0 )
		{
			return FString::Printf( *LOCTEXT("ShowingNoFunctions", "No matching items (%d total)").ToString(), TotalClassesAndFunctions );
		}
		else
		{
			return FString::Printf( *LOCTEXT("ShowingOnlySomeFunctions", "Showing %d of %d items").ToString(), SignatureToTreeItemMap.Num(), TotalClassesAndFunctions );
		}
	}
	...
	FString SCodeView::GetFilterButtonToolTip() const
	{
	...
	FString SCodeView::GetSelectedModeText() const
	{
