...
namespace PackageTools
{
	 ...
	 */
	bool SavePackages( const TArray<UPackage*>& PackagesToSave, bool bUnloadPackagesAfterSave, FString* pLastSaveDirectory /*=NULL*/ )
	{
		...
		for( int32 PackageIndex = 0 ; PackageIndex < PackagesToSave.Num() ; ++PackageIndex )
		{
			...
			if ( Package != NULL )
			{
				Packages.AddUnique( Package->GetOutermost() ? (UPackage*)Package->GetOutermost() : Package );
			}
		}

		// Packages must be fully loaded before they can be saved.
		if( !HandleFullyLoadingPackages( Packages, NSLOCTEXT("UnrealEd", "Save", "Save") ) )
		{
			return false;
		}

		// Assume that all packages were saved.
		bool bAllPackagesWereSaved = true;
		...
		FileTypes += FString::Printf( TEXT("|(*%s)|*%s"), *FPackageName::GetMapPackageExtension(), *FPackageName::GetMapPackageExtension() );
		...
		for(int32 i=0; i<Packages.Num(); i++)
		{
			...
			if ( PackageToSave != NULL )
			{
				...
				if( TheWorld )
				{
					FMessageDialog::Open( EAppMsgType::Ok, FText::Format(NSLOCTEXT("UnrealEd", "Error_CantSaveMapViaGB", "Can't save map files via the browser. Not saving '{0}'."), FText::FromString(PackageToSave->GetName())) );
				...
				else
				{
					...
					if( FPackageName::DoesPackageExist( PackageName, NULL, &ExistingFile ) )
					{
					...
					{
						ensureMsg(false, TEXT("All files should be found in the package file cache"));
						...
						const FString File = FString::Printf( TEXT("%s%s"), *PackageName, *FPackageName::GetAssetPackageExtension() );
						UE_LOG(LogPackageTools, Log, TEXT("NO_EXISTING: %s, %s, %s"), *ExistingFile, *File, *LastSaveDirectory);
						...
						while( !bFilenameIsValid )
						{
							...
							if( bSave )
							{
								LastSaveDirectory = FPaths::GetPath(SaveFilenames[0]);
								SaveFileName = SaveFilenames[0];
								
								// If the supplied file name is missing an extension then give it the default package
								// file extension.
								if( SaveFileName.Len() > 0 && FPaths::GetExtension(SaveFileName).Len() == 0 )
								{
									SaveFileName += FPackageName::GetAssetPackageExtension();
								}						
					...
					{
						const FScopedBusyCursor BusyCursor;
						UE_LOG(LogPackageTools, Log, TEXT("saving as %s"), *SaveFileName);
						if( IFileManager::Get().IsReadOnly( *SaveFileName ) )
						{
						...
						else
						{
							...
							{
								PackagesToUnload.Add( PackageToSave );
							}
						}
					}
					else
