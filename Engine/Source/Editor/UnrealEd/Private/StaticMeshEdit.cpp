...
UStaticMeshFactoryT3D::UStaticMeshFactoryT3D(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
	SupportedClass = UStaticMesh::StaticClass();
	Formats.Add(TEXT("t3d;Static Mesh"));
...
FText UStaticMeshFactoryT3D::GetDisplayName() const
{
	return LOCTEXT("StaticMeshFactoryT3DDescription", "T3D Static Mesh");
}

UObject* UStaticMeshFactoryT3D::FactoryCreateText
(
	UClass*				Class,
	UObject*			InParent,
	FName				Name,
...
)
{
	...
	FStaticMeshComponentRecreateRenderStateContext RecreateRenderStateContext(FindObject<UStaticMesh>(InParent,*Name.ToString()));

	// Make sure rendering is done - so we are not changing data being used by collision drawing.
	FlushRenderingCommands();

	if(FCString::Stricmp(Type,TEXT("T3D")) == 0)
	{
		if(GetBEGIN(&BufferTemp,TEXT("PolyList")))
		{
			UModelFactory*	ModelFactory = new UModelFactory(FPostConstructInitializeProperties());
			UModel*			Model = (UModel*) ModelFactory->FactoryCreateText(UModel::StaticClass(),InParent,NAME_None,RF_NoFlags,NULL,TEXT("T3D"),Buffer,BufferEnd,Warn);
			...
			if(Model)
			{
				...
				if(StaticMesh)
				{
					// @todo AssetImportData make a data class for t3d assets
					StaticMesh->AssetImportData = ConstructObject<UAssetImportData>(UAssetImportData::StaticClass(), StaticMesh);
					StaticMesh->AssetImportData->SourceFilePath = FReimportManager::SanitizeImportFilename(CurrentFilename, StaticMesh);
					StaticMesh->AssetImportData->SourceFileTimestamp = IFileManager::Get().GetTimeStamp(*CurrentFilename).ToString();
				}
