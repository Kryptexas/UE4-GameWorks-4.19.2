...
void FAnimationEditorCommands::RegisterCommands()
{
	...
	UI_COMMAND(ExportToFBX_Source, "Source", "Export current animation source to FBX", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ExportToFBX_Play, "Play", "Export current animation from from Play on the current preview mesh, including retargeting, post process graph, or anything you see on the preview mesh.", EUserInterfaceActionType::Button, FInputChord());
