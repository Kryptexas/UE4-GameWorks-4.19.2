...
void UGameplayTagsK2Node_MultiCompareGameplayTagContainer::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	for (int32 Index = 0; Index < NumberOfPins; ++Index)
	{
		...
		const UFunction* Function = UBlueprintGameplayTagLibrary::StaticClass()->FindFunctionByName(TEXT("DoesContainerMatchAllTagsInContainer"));
...
void UGameplayTagsK2Node_MultiCompareGameplayTagContainer::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
FText UGameplayTagsK2Node_MultiCompareGameplayTagContainer::GetTooltipText() const
{
	return NSLOCTEXT("K2Node", "MultiCompareTagContainer_ToolTip", "Sets the an output for each input value");
}

FText UGameplayTagsK2Node_MultiCompareGameplayTagContainer::GetMenuCategory() const
{
...
FString UGameplayTagsK2Node_MultiCompareGameplayTagContainer::GetUniquePinName()
{
	FString NewPinName;
	int32 Index = 0;
	while (true)
