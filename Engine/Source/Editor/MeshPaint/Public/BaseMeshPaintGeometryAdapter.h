...
public:
	...
	virtual const TArray<FVector>& GetMeshVertices() const;
	virtual const TArray<uint32>& GetMeshIndices() const;
	virtual void GetVertexPosition(int32 VertexIndex, FVector& OutVertex) const;
	virtual TArray<uint32> SphereIntersectTriangles(const float ComponentSpaceSquaredBrushRadius, const FVector& ComponentSpaceBrushPosition, const FVector& ComponentSpaceCameraPosition, const bool bOnlyFrontFacing) const;
	virtual void GetInfluencedVertexIndices(const float ComponentSpaceSquaredBrushRadius, const FVector& ComponentSpaceBrushPosition, const FVector& ComponentSpaceCameraPosition, const bool bOnlyFrontFacing, TSet<int32> &InfluencedVertices) const;
