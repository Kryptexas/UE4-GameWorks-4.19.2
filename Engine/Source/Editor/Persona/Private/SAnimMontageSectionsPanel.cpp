...
void SAnimMontageSectionsPanel::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			.AreaTitle( LOCTEXT("Sections", "Sections").ToString() )
			.BodyContent()
			[
				SNew( SBorder )
		...
		MontageSlots->AddSlot()
			...
			[
				...
				[				
					...
					.Text( LOCTEXT("CreateDefault", "Create Default").ToString() )
					.ToolTipText( LOCTEXT("CreateDefaultToolTip", "Reconstructs section ordering based on start time").ToString() )
				...
				[				
					SNew(SButton)
					.Visibility( EVisibility::Visible )
					.Text( LOCTEXT("Clear", "Clear").ToString() )
					.ToolTipText( LOCTEXT("ClearToolTip", "Resets section orderings").ToString() )
		...
		MontageSlots->AddSlot()
			...
			[
				...
				[				
					SNew(SButton)
					.Visibility( EVisibility::Visible )
					.Text( LOCTEXT("PreviewAll", "Preview All Sections").ToString() )
					.ToolTipText( LOCTEXT("PreviewAllToolTip", "Preview all sections in order they are").ToString() )
			...
			MontageSlots->AddSlot()
				...
				[
					...
					[				
						SNew(SButton)
						.Visibility( EVisibility::Visible )
						.Text( LOCTEXT("Preview", "Preview").ToString() )
						.ToolTipText( LOCTEXT("PreviewToolTip", "Preview this track").ToString() )
