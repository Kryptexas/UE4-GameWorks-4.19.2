...
enum EVREventType
{
	...
	VREvent_TrackedDeviceActivated		= 100, // param1 = Device index
	VREvent_TrackedDeviceDeactivated	= 101, // param1 = Device index
	VREvent_TrackedDeviceUpdated		= 102, // param1 = Device index
};
...
struct VREvent_t
{
	...
	uint32_t param1; // Not used yet. This is here to reserve space in the interface.
...
static const char * const IVRSystem_Version = "IVRSystem_001";
...
static const char * const IVRChaperone_Version = "IVRChaperone_001";
...
struct Compositor_FrameTiming
{
	...
	float prediction;
	float submitBegin, workFinished, presentFinished;
...
public:
	...
	*
	* Note: Currently only Compositor_DeviceType_D3D11 is supported
	*/
	...
	* can be useful if you mostly want the default values but want to change a few settings. */
	virtual Compositor_OverlaySettings GetOverlayDefaults() = 0;
	...
	* can be png or jpg, and should not be bigger than 1920x1080 */
	...
	/** Returns frame stats and history.  Will return oldest timing info if nFramesAgo is larger than the stored history. */
	virtual Compositor_FrameTiming GetFrameTiming( uint32_t unFramesAgo = 0 ) = 0;
...
static const char * const IVRCompositor_Version = "IVRCompositor_004";
...
*/
VR_INTERFACE class IVRSystem *VR_CALLTYPE VR_Init( HmdError *peError );
...
VR_INTERFACE const char *VR_CALLTYPE VR_GetStringForHmdError( HmdError error );

/** Returns the interface of the specified version. This method must be called after VR_Init. The
* pointer returned is valid until VR_Shutdown is called.
*/
VR_INTERFACE void *VR_CALLTYPE VR_GetGenericInterface( const char *pchInterfaceVersion, HmdError *peError );
