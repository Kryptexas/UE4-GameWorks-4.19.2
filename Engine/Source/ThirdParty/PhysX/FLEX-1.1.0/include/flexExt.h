...
// A vector type that wraps a FlexBuffer, behaves like a standard vector for POD types (no construction)
// The vector must be mapped using map() before any read/write access to elements or resize operation

template <typename T>
struct FlexVector
{
	FlexVector(FlexLibrary* lib, int size=0) : flexLib(lib), buffer(NULL), mappedPtr(NULL), count(0), capacity(0)
	{
		if (size)
		{
			resize(size);
	...
	FlexVector(FlexLibrary* lib, const T* ptr, int size) : flexLib(lib), buffer(NULL), mappedPtr(NULL), count(0), capacity(0)
	{
		assign(ptr, size);
		unmap();
	}
	...
	void map(int flags=eFlexMapWait)
	{
		if (!buffer)
			return;

		assert(!mappedPtr);
		mappedPtr = (T*)flexMap(buffer, flags);
	}
	...
	void reserve(int minCapacity)
	{
		if (minCapacity > capacity)
		{
			...
			FlexBuffer* newBuf = flexAllocBuffer(flexLib, newCapacity, sizeof(T), eFlexBufferHost);

			// copy contents to new buffer			
			void* newPtr = flexMap(newBuf, eFlexMapWait);
...
/**
 * Helper struct for storing the state of a moving frame, see flexExtMovingFrameInit()
 */
 ...
 * The FlexExtMovingFrame provides a way to include or remove these inertial forces. The basic usage is as follows:
 *  
 \code{.c}

	FlexExtMovingFrame frame;
	flexExtMovingFrameInit(&frame, initialTranslation, initialRotation);
	...
	while(simulating)
	{
		...
		// update the frame
		flexExtMovingFrameUpdate(frame, newPosition, newRotation, dt);

		// apply inertial forces and update particles
		flexExtMovingFrameApply(frame, particlePositions, particleVelocities, numParticles, linearInertiaScale, angularInertiaScale, dt);
	}

 \endcode
 
 * @param[in] frame A pointer to a user-allocated FlexExtMovingFrame struct
 ...
 */
FLEX_API void flexExtMovingFrameInit(FlexExtMovingFrame* frame, const float* worldTranslation, const float* worldRotation);
 ...
 */
FLEX_API void flexExtMovingFrameUpdate(FlexExtMovingFrame* frame, const float* worldTranslation, const float* worldRotation, float dt);
 ...
 * @param[in] dt The time that elapsed since the last call to the frame update, should match the value passed to flexExtMovingFrameUpdate()
 */
FLEX_API void flexExtMovingFrameApply(FlexExtMovingFrame* frame, float* positions, float* velocities, int numParticles, float linearScale, float angularScale, float dt);


/** 
 * Represents a group of particles and constraints, each asset 
 * can be instanced into a container using flexExtCreateInstance()
 */
struct FlexExtAsset
{	
	// particles
	...
	int mMaxParticles;				//!< Maximum number of particles, allows extra space for tearable assets which duplicate particles
	...
	// faces for cloth
	int* mTriangleIndices;			//!< Indexed triangle mesh indices for clothing
 ...
 */
typedef struct FlexExtContainer FlexExtContainer;
 ...
 * This can be used to create simulation meshes from render meshes, and is typically done as a pre-pass before calling flexExtCreateClothFromMesh().
 *
 ...
 */
FLEX_API FlexExtContainer* flexExtCreateContainer(FlexLibrary* lib, FlexSolver* solver, int maxParticles);
 ...
 * remain valid flexExtUnmapParticleData() is called.
 *
  @param container The container whose data should be accessed
 */
FLEX_API FlexExtParticleData flexExtMapParticleData(FlexExtContainer* container);
FLEX_API void flexExtUnmapParticleData(FlexExtContainer* container);

struct FlexExtTriangleData
{
 ...
 */
FLEX_API FlexExtTriangleData flexExtMapTriangleData(FlexExtContainer* container);

/** 
 * Unmap triangle data, see flexExtMapTriangleData()
 */
FLEX_API void flexExtUnmapTriangleData(FlexExtContainer* container);

struct FlexExtShapeData
{
 ...
 * @param container The container to retrive from
 */
FLEX_API FlexExtShapeData flexExtMapShapeData(FlexExtContainer* container);

/** 
 * Unmap shape transform data, see flexExtMapShapeData()
 */
FLEX_API void flexExtUnmapShapeData(FlexExtContainer* container);
 ...
 * @param[in] particleData Pointer to a mapped particle data struct, returned from flexExtMapParticleData()
 ...
 */
FLEX_API FlexExtInstance* flexExtCreateInstance(FlexExtContainer* container,  FlexExtParticleData* particleData, const FlexExtAsset* asset, const float* transform, float vx, float vy, float vz, int phase, float invMassScale);
 ...
 *
    \code{.c}
		...
		FlexExtParticleData data flexExtMapParticleData();

		// CPU particle processing
		ProcessParticles(data);

		// unmap data
		flexExtUnmapParticleData();
  ...
  @param[in] enableTimers Whether to record detailed timers, see flexUpdateSolver()
 */
FLEX_API void flexExtTickContainer(FlexExtContainer* container, float dt, int numSubsteps, bool enableTimers=false);
 ...
 * expected to be called in the following sequence: flexExtPushToDevice, flexUpdateSolver, flexExtPullFromDevice
 * @param[in] container The container to update
 */ 
...
struct FlexExtForceField
{
	...
	FlexExtForceMode mMode;	//!< Mode of field application
 ...
 */
typedef struct FlexExtForceFieldCallback FlexExtForceFieldCallback;

/**
 * Create a FlexExtForceFieldCallback structure, each callback is associated with the
 * passed in solver once the flexExtSetForceFields() is called.
 *
 * @param[in] solver A valid solver created with flexCreateSolver()
 * @return A pointer to a callback structure
 */
FLEX_API FlexExtForceFieldCallback* flexExtCreateForceFieldCallback(FlexSolver* solver);

/**
 * Destroy the force field callback
 *
 * @param[in] callback A valid solver created with flexExtCreateForceFieldCallback()
 */
FLEX_API void flexExtDestroyForceFieldCallback(FlexExtForceFieldCallback* callback);
 ...
 */
FLEX_API void flexExtSetForceFields(FlexExtForceFieldCallback* callback, const FlexExtForceField* forceFields, int numForceFields);
