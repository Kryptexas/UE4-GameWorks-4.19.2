...
#ifndef FLEX_DEVICE_H
#define FLEX_DEVICE_H
...
/** \file 
 * The flexDevice is a helper library that performs some
 * initialization tasks related to GPU device management.
 * It is an optional, closed source, component of Flex. The 
 * flexDevice can be used to query the PhysX control panel for
 * the selected "PhysX" GPU and to create an optimized
 * CUDA context. See the FlexDemo for an example of how to use
 * the device API.
 */
 ...
 * DXGI device index to flexInit().
 */
FLEX_API int flexDeviceGetSuggestedOrdinal();
 ...
 */
FLEX_API bool flexDeviceCreateCudaContext(int ordinal);
 ...
 */
FLEX_API void flexDeviceDestroyCudaContext();
