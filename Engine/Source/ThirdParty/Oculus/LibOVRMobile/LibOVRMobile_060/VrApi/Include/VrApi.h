...
for ( ; ; )
{
	...
	// Use the ovrHmdInfo suggested FOV and suggested resolution to setup a projection
	// matrix and eye render targets.
	const ovrHmdInfo hmdInfo = vrapi_GetHmdInfo( &java );

	// Setup the ovrModeParms.
	...
	for ( long long frameIndex = 1; ; frameIndex++ )
	{
		...
		const ovrTracking tracking = vrapi_GetPredictedTracking( ovr, predictedDisplayTime );

		// Advance the simulation based on the predicted display time.

		// Render eye images and setup ovrFrameParms using ovrTracking.
		ovrFrameParms frameParms = vrapi_DefaultFrameParms( &java, VRAPI_FRAME_INIT_DEFAULT, 0 );
		frameParms.FrameIndex = frameIndex;
		for ( int eye = 0; eye < 2; eye++ )
		{
			frameParms.Layers[VRAPI_FRAME_LAYER_TYPE_WORLD].Images[eye].TexId = ;	// eye image
			frameParms.Layers[VRAPI_FRAME_LAYER_TYPE_WORLD].Images[eye].HeadPose = tracking.HeadPose;
		}
...
VR mode when the phone is docked into the headset. A non-stereo app shoud not be burdened
with a SCHED_FIFO device manager thread (for sensor input) while not in VR mode. In other
words, there is no sensor input until the phone is docked and the app enters VR mode.
...
used for rendering. The reason for this is that the VrApi allows for one frame of GPU overlap
...
// Returns HMD specific details.
// Can be called any time from any thread.
OVR_VRAPI_EXPORT ovrHmdInfo vrapi_GetHmdInfo( const ovrJava * java );
...
OVR_VRAPI_EXPORT void vrapi_Initialize( const ovrInitParms * initParms );
...
// thread scheduling, and sets video options. This should be called after
// vrapi_Initialize(), when the app is both resumed and has a valid window
// surface. This must be called from a thread that has an OpenGL ES context
// current on the active Android window surface. The context of the calling
...
OVR_VRAPI_EXPORT void ovr_AdjustClockLevels( ovrMobile * ovr, int cpuLevel, int gpuLevel );

//-----------------------------------------------------------------
// Tracking
//-----------------------------------------------------------------

// Predict the time at which the next set of eye images will be displayed.
...
// Returns an absolute system time in seconds.
