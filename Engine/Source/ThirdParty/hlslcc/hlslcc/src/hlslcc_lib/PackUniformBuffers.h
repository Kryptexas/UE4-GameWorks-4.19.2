...
extern SHADERCOMPILERCOMMON_API void FixRedundantCasts(exec_list* IR);
...
extern SHADERCOMPILERCOMMON_API void RemovePackedUniformBufferReferences( exec_list* Instructions, _mesa_glsl_parse_state* ParseState, TVarVarMap& UniformMap );
...
//			float4 Value1;
//		};
extern SHADERCOMPILERCOMMON_API void FlattenUniformBufferStructures(exec_list* Instructions, _mesa_glsl_parse_state* ParseState);
 ...
 */
extern SHADERCOMPILERCOMMON_API void PackUniforms(exec_list* Instructions, _mesa_glsl_parse_state* ParseState, bool bFlattenStructure, bool bGroupFlattenedUBs, TVarVarMap& OutUniformMap);

// Expand any full assignments (a = b) to per element (a[0] = b[0]; a[1] = b[1]; etc) so the array can be split
extern SHADERCOMPILERCOMMON_API bool ExpandArrayAssignments(exec_list* ir, _mesa_glsl_parse_state* State);

// Removes the SamplerStates from hlsl while making a map for the textures it references
extern SHADERCOMPILERCOMMON_API bool ExtractSamplerStatesNameInformation(exec_list* Instructions, _mesa_glsl_parse_state* ParseState);

// Expands matrices to arrays to help on some cases (e.g., non-square matrices on ES 2)
extern SHADERCOMPILERCOMMON_API bool ExpandMatricesIntoArrays(exec_list* Instructions, _mesa_glsl_parse_state* ParseState);
