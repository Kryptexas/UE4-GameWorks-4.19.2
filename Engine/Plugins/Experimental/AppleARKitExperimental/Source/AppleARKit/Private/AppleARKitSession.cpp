...
bool UAppleARKitSession::Run()
{
	...
	UAppleARKitWorldTrackingConfiguration* Configuration = UAppleARKitWorldTrackingConfiguration::StaticClass()->GetDefaultObject< UAppleARKitWorldTrackingConfiguration >();

	// Run with that
	return RunWithConfiguration(Configuration);
}

bool UAppleARKitSession::RunWithConfiguration(const UAppleARKitConfiguration* InConfiguration)
{
	// Already running?
	if (IsRunning())
	{
	...
	ARConfiguration* Configuration = ToARConfiguration(InConfiguration);
