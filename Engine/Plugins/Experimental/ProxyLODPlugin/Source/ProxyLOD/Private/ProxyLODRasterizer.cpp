...
namespace ProxyLOD
{
	...
	bool TDilateGrid(TGrid<DataType>& RasterGrid)
	{
		typedef TGrid<DataType>    DataGrid;

		FIntPoint Size = RasterGrid.Size();
		const int TexelCount = Size.X * Size.Y;
		TGrid<int32> TopologyGrid(Size.X, Size.Y);

		int* TopologyData = TopologyGrid.GetData();
		DataType* RasterData = RasterGrid.GetData();

		// Copy the current topology of the RasterGrid into the TopologyGrid 
		Parallel_For(FIntRange(0, TexelCount),
			[TopologyData, RasterData](const FIntRange& Range)
		{
			for (int32 r = Range.begin(), R = Range.end(); r < R; ++r)
			{
				TopologyData[r] = RasterData[r].TriangleId > -1 ? 1 : 0;
			}
		});

		// Count the number of texels that are full.
		int FullCount =
			Parallel_Reduce(FIntRange(0, TexelCount), 0, [TopologyData](const FIntRange& Range, const int& InSum)->int
		{
			int LocalSum = 0;
			for (int32 r = Range.begin(), R = Range.end(); r < R; ++r)
			{
			...
			return LocalSum + InSum;
				...
				[](int32 SumA, int32 SumB)->int
		{
			return SumA + SumB;
		});

		int InActiveCount = Size.X * Size.Y - FullCount;
		if (InActiveCount != 0)
		{

			// Do the dilation of the interior
			Parallel_For(FIntRange2d(1, Size.X - 1, 1, Size.Y - 1),
				[&TopologyGrid, &RasterGrid](const FIntRange2d Range)
			{

				for (int32 i = Range.rows().begin(), I = Range.rows().end(); i < I; ++i)
				{
					for (int32 j = Range.cols().begin(), J = Range.cols().end(); j < J; ++j)
					{
						// this is an empty cell
						if (TopologyGrid(i, j) == 0)
						{
							// this order resolves the case where information could flow from multiple sides.
							if (TopologyGrid(i - 1, j) == 1)
							{

								RasterGrid(i, j) = RasterGrid(i - 1, j);
							}
							else if (TopologyGrid(i, j - 1) == 1)
							{

								RasterGrid(i, j) = RasterGrid(i, j - 1);
							}
							else if (TopologyGrid(i + 1, j) == 1)
							{
	...
	bool TDilateGrid(TGridWrapper<ValueType>& RasterGrid, TGrid<int32>& TopologyGrid)
	{
		checkSlow(TopologyGrid.Size() == RasterGrid.Size());

		typedef TGridWrapper<ValueType>    DataGrid;
