	...
	private:
		...
		*/
		void NextAsset()
		{
			...
			{
				TestExecutionInfo->Errors[ErrorIndex].Message = FString::Printf(TEXT("%s: %s"), *LogTag, *TestExecutionInfo->Errors[ErrorIndex].Message);
			}
	...
	private:
		...
		*/
		bool EndSection()
		{
			...
			{
				TestExecutionInfo->Errors.Insert(FAutomationEvent(FString::Printf(TEXT("\n\n--%s Pass--"), *SectionName)), SectionErrorCount);
			}
...
#undef IMPORT_ASSET_WITH_FACTORY
		...
		bool Blueprint_Placement_Part1()
		{
			if (BlueprintObject)
			{
				...
				if (PlacedBlueprint)
				{
					...
					Test->AddLogItem(TEXT("Updated string variable value"));
				}
		...
		*/
		bool Blueprint_SetBreakpoint_Part1()
		{
			if (BlueprintObject)
			{
				...
				Test->AddLogItem(TEXT("Set a breakpoint on the PrintString node"));
				EditorBuildPromotionTestUtils::StartPIE(true);
		...
		*/
		bool Blueprint_SetBreakpoint_Part2()
		{
			if (BlueprintObject)
			{
				...
				{
					//Success!
					Test->AddLogItem(TEXT("Hit the PrintString breakpoint"));
					DelayHelper.Reset();
					return true;
				}
				return false;
		...
		*/
		bool Blueprint_SetBreakpoint_Part3()
		{
			if (BlueprintObject)
			{
				Test->AddLogItem(TEXT("Clearing the breakpoint"));
		...
		*/
		bool Blueprint_LevelScript_Part1()
		{
			if (BlueprintObject)
			{
				...
				Test->AddLogItem(TEXT("Opened the level script blueprint"));
			}
			return true;
		}
		...
		*/
		bool Blueprint_LevelScript_Part2()
		{
			if (BlueprintObject)
			{
				...
				Test->AddLogItem(TEXT("Copied the blueprint event nodes"));
				Test->AddLogItem(TEXT("Pasted the nodes into to level script"));
				...
				//We are expecting errors here.  So copy any new errors to the logs section
				{
					...
					{
						TestExecutionInfo->LogItems.Add(TestExecutionInfo->Errors[i].Message);
					}
