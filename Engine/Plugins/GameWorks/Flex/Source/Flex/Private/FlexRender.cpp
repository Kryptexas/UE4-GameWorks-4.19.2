	...
	: FStaticMeshSceneProxy(Component, false)
{
	...
	const FStaticMeshVertexFactories& VFs = Component->GetStaticMesh()->RenderData->LODVertexFactories[0];
...
void FFlexMeshSceneProxy::UpdateSoftTransforms(const FFlexShapeTransform* NewTransforms, int32 NumShapes)
{
	...
	const FPositionVertexBuffer& Positions = FlexComponent->GetStaticMesh()->RenderData->LODResources[0].VertexBuffers.PositionVertexBuffer;
	const FStaticMeshVertexBuffer& Vertices = FlexComponent->GetStaticMesh()->RenderData->LODResources[0].VertexBuffers.StaticMeshVertexBuffer;
