	...
	float NoiseAmplitude, float NoiseFrequency, int32 NoiseOctaveNumber, FVector SamplingInterval)
{
	...
	slConfig.noise.samplingInterval = physx::PxVec3(SamplingInterval.X, SamplingInterval.Y, SamplingInterval.Z);
	if (FractureSession->FractureTool->getChunkDepth(FractureChunkId) == 0)
	{
		IsReplace = false;
	}
	...
	float NoiseAmplitude, float NoiseFrequency, int32 NoiseOctaveNumber, FVector SamplingInterval)
{
	...
	CutoutConfig.noise.samplingInterval = physx::PxVec3(SamplingInterval.X, SamplingInterval.Y, SamplingInterval.Z);
	if (Pattern != nullptr)
	{
	...
	float NoiseAmplitude, float NoiseFrequency, int32 NoiseOctaveNumber, FVector SamplingInterval)
{
	...
	NoiseConfig.samplingInterval = physx::PxVec3(SamplingInterval.X, SamplingInterval.Y, SamplingInterval.Z);
	if (FractureSession->FractureTool->getChunkDepth(FractureChunkId) == 0)
	{
		IsReplace = false;
	}
