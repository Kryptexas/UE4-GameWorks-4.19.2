...
void SSubversionSourceControlSettings::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("RepositoryLabel", "Repository").ToString())
				.ToolTipText(LOCTEXT("RepositoryLabel_Tooltip", "Address of SVN repository").ToString())
				.Font(Font)
			]
			+SVerticalBox::Slot()
			.FillHeight(1.0f)
			...
			[
				...
				.ToolTipText(LOCTEXT("UserNameLabel_Tooltip", "SVN username").ToString())
				.Font(Font)
			]
			+SVerticalBox::Slot()
			.FillHeight(1.0f)
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("LabelsDirectoryLabel", "Labels Directory").ToString())
				.ToolTipText(LOCTEXT("LabelsDirectoryLabel_Tooltip", "Relative path to repository root where labels/tags are stored. For example, if the labels/tags were to be stored in 'http://repo-name/tags/', then the path here would be 'tags/'").ToString())
				.Font(Font)
			]
			+SVerticalBox::Slot()
			.FillHeight(1.0f)
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("PasswordLabel", "Password").ToString())
				.ToolTipText(LOCTEXT("Password_Tooltip", "Enter your password here if your repository requires it.\nYour credentials will be stored by Subversion once you have successfully logged on, so you won't have to enter it again.").ToString())
				.Font(Font)
			]
		]
		+SHorizontalBox::Slot()
