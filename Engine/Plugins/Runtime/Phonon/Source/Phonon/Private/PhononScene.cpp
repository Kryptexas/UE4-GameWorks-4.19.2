...
namespace Phonon
{
	...
	void LoadScene(UWorld* World, IPLhandle* PhononScene, TArray<IPLhandle>* PhononStaticMeshes)
	{
		...
		UE_LOG(LogPhonon, Log, TEXT("Loading Phonon scene."));

		IPLSimulationSettings SimulationSettings;
		SimulationSettings.sceneType = IPL_SCENETYPE_PHONON;
		IPLerror IplResult = iplCreateScene(Phonon::GlobalContext, nullptr, SimulationSettings, CalculateNumMaterials(World), PhononScene);
		if (IplResult != IPL_STATUS_SUCCESS)
		{
			UE_LOG(LogPhonon, Warning, TEXT("Error creating Phonon scene."));
			return;
		}

		LoadStaticMeshActors(World, *PhononScene, PhononStaticMeshes);

		if (GetDefault<UPhononSettings>()->ExportLandscapeGeometry)
		{
			LoadLandscapeActors(World, *PhononScene, PhononStaticMeshes);
		}

		if (GetDefault<UPhononSettings>()->ExportBSPGeometry)
		{
	...
	static void LoadStaticMeshActors(UWorld* World, IPLhandle PhononScene, TArray<IPLhandle>* PhononStaticMeshes)
	{
		...
		UE_LOG(LogPhonon, Log, TEXT("Loading static mesh actors."));

		// There are size(MaterialPresets) + 3 fixed material slots.
		auto CustomMaterialIdx = MaterialPresets.Num() + 3;
		...
		for (TActorIterator<AStaticMeshActor> AStaticMeshItr(World); AStaticMeshItr; ++AStaticMeshItr)
		{
				...
				AStaticMeshItr->GetStaticMeshComponent()->GetStaticMesh()->HasValidRenderData())
			{
				...
				auto PhononMaterialComponent = static_cast<UPhononMaterialComponent*>(
					AStaticMeshItr->GetComponentByClass(UPhononMaterialComponent::StaticClass()));

				auto DefaultMaterial = GetDefault<UPhononSettings>()->StaticMeshMaterialPreset;
				auto MaterialIdx = 0;

				if (PhononMaterialComponent)
				{
					switch (PhononMaterialComponent->MaterialPreset)
					{
					case EPhononMaterial::CUSTOM:
						MaterialIdx = CustomMaterialIdx++;
						iplSetSceneMaterial(PhononScene, MaterialIdx, PhononMaterialComponent->GetMaterialPreset());
						break;
					default:
						MaterialIdx = static_cast<int32>(PhononMaterialComponent->MaterialPreset);
						break;
					}
				}
				else
				{
					...
					MaterialIdx = MaterialPresets.Num();
				}
	...
	static void LoadBSPGeometry(UWorld* World, IPLhandle PhononScene, TArray<IPLhandle>* PhononStaticMeshes)
	{
		...
		UE_LOG(LogPhonon, Log, TEXT("Loading BSP geometry."));
		...
		{
			IplVertices.Add(Phonon::UnrealToPhononIPLVector3(WorldVertex));
		}
	...
	static void LoadLandscapeActors(UWorld* World, IPLhandle PhononScene, TArray<IPLhandle>* PhononStaticMeshes)
	{
		...
		UE_LOG(LogPhonon, Log, TEXT("Loading landscape actors."));
			...
			for (auto It = LandscapeInfo->XYtoComponentMap.CreateIterator(); It; ++It)
			{
				...
				for (auto y = 0; y < Component->ComponentSizeQuads; ++y)
				{
					for (auto x = 0; x < Component->ComponentSizeQuads; ++x)
					{
						...
						IplVertices.Add(Phonon::UnrealToPhononIPLVector3(CDI.GetWorldVertex(x, y)));
						IplVertices.Add(Phonon::UnrealToPhononIPLVector3(CDI.GetWorldVertex(x, y + 1)));
						IplVertices.Add(Phonon::UnrealToPhononIPLVector3(CDI.GetWorldVertex(x + 1, y + 1)));
						IplVertices.Add(Phonon::UnrealToPhononIPLVector3(CDI.GetWorldVertex(x + 1, y)));
		...
		TArray<IPLint32>& IplMaterialIndices, TArray<IPLhandle>* PhononStaticMeshes)
	{
		...
		{
			UE_LOG(LogPhonon, Log, TEXT("Registering new mesh with %d verts."), IplVertices.Num());
			...
			if (IplResult != IPL_STATUS_SUCCESS)
			{
				UE_LOG(LogPhonon, Warning, TEXT("Error adding a new object to the acoustic scene."));
				return;
			}
		...
		{
			UE_LOG(LogPhonon, Warning, TEXT("Skipping mesh registration because no vertices were found."));
		}
	}
	...
	static void SetCommonSceneMaterials(IPLhandle PhononScene)
	{
		...
		iplSetSceneMaterial(PhononScene, MaterialPresets.Num(), GetDefault<UPhononSettings>()->GetDefaultStaticMeshMaterial());
		iplSetSceneMaterial(PhononScene, MaterialPresets.Num() + 1, GetDefault<UPhononSettings>()->GetDefaultBSPMaterial());
		iplSetSceneMaterial(PhononScene, MaterialPresets.Num() + 2, GetDefault<UPhononSettings>()->GetDefaultLandscapeMaterial());
	}
}
