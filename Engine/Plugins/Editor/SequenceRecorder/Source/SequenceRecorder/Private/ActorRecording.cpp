...
void UActorRecording::StartRecordingActorProperties(ULevelSequence* CurrentSequence, float CurrentSequenceTime)
{
	if(CurrentSequence != nullptr)
	{
		...
		else if(UBlueprint* ActorBlueprint = Cast<UBlueprint>(ActorClass->ClassGeneratedBy))
		{
		...
		{
			UActorFactory* FactoryToUse = GEditor->FindActorFactoryForActorClass(ActorClass);
			if(FactoryToUse)
			{
				Blueprint = FactoryToUse->CreateBlueprint(Actor, MovieScene, *BlueprintName);
			}
		}
...
bool UActorRecording::StopRecording(ULevelSequence* CurrentSequence)
{
	...
	if(bNewComponentAddedWhileRecording)
	{
		UMovieScene* MovieScene = CurrentSequence->GetMovieScene();
		check(MovieScene);
